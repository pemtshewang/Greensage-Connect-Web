...
const FormSchema = z.object({
  ...
  wifiPassword: z.string().min(8, { message: "The password should be min of 8 characters" }),
});

const getIrrigationControllers = async (id: string) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/controller/irrigation-controllers`, {
    method: "POST",
    body: JSON.stringify({
      userId: id
    })
...
const getGreenhouseControllers = async (id: string) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/controller/greenhouse-controllers`, {
    method: "POST",
    body: JSON.stringify({
      userId: id
    })
...
const getUserDetail = async (id: string) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/controller/generate-code/user-config`, {
    method: "POST",
    body: JSON.stringify({
      userId: id
    })
...
const generateCode = async (credentials: z.infer<typeof FormSchema>) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/controller/generate-code`, {
    method: "POST",
...
export default function GenerateCodeForm({ id }: { id: string }) {
  ...
  const [greenhouseControllers, setGreenhouseControllers] = useState<IControllerDetail[]>([]);
  const [irrigationControllers, setIrrigationControllers] = useState<IControllerDetail[]>([]);
  ...
  useEffect(() => {
    if (controllerId !== "") {
      ...
      getUserDetail(id).then((res) => {
        ...
        form.setValue("brokerPassword", res?.password)
        ...
        controllerCode && (
          <div className="flex space-x-4 absolute right-6 z-10">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button onClick={() => handleCopyToClipboard(controllerCode)} variant="outline" >
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button onClick={() => downloadCode(controllerCode)} variant="outline" >
                    <Icons.downloadIcon />
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
        ...
        controllerCode ? (
          <pre>
            <ScrollArea className="h-[80vh] p-5">
              <SyntaxHighlighter wrapLines={true} showLineNumbers startingLineNumber={0} language="cpp" style={docco}>
                {controllerCode}
              </SyntaxHighlighter>
            </ScrollArea>
          </pre>
          ...
          <>
            <div>
              {
                fetching ? (
                  <div className="flex flex-col justify-center items-center  lg:h-[85vh]">
                    ...
                    <h2 className="animate-pulse delay-1000 tracking-wide">Fetching all available controllers for user</h2>
                  </div>
                ) : (
                  <div>
                    <div className="flex space-x-5">
                      ...
                      <Select onValueChange={(item) => {
                        ...
                        <SelectTrigger disabled={!showList} className="lg:max-w-[300px]">
                        ...
                        <SelectContent>
                          {
                            type === "greenhouse" ? (
                              greenhouseControllers.map((controller) => (
                                <SelectItem value={controller.controllerId}>{controller.name} : {controller.controllerId}</SelectItem>
                              ))
                            ) : (
            ...
            </div>
            {
              formLoading ? (
                <div className="flex flex-col justify-center items-center  lg:h-[85vh]">
                  <Icons.codeGenerationLoading width={72} height={72} />
                  <h2 className="animate-pulse delay-1000 tracking-wide">Generating Form for Code</h2>
                </div>
              ) : recievingCode ? (
                <div className="flex flex-col justify-center items-center  lg:h-[85vh]">
                  <Icons.codeGenerationLoading width={72} height={72} />
                  <h2 className="animate-pulse delay-1000 tracking-wide">Generating code for the controller</h2>
                </div>
              ) : controllerId ?
                (
                  <ScrollArea className="h-[80vh] rounded-md p-4 border mt-4">
                    <Form {...form} >
                      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-3 ml-2">
                        <FormField
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                Enter the broker password for user or keep it as default
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                The controller will use this ssid to connect to internet
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                The password will be used for wifi authentication to connect to interent
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Enter the access point name for your controller</FormLabel>
                              <FormControl>
                                <Input
                                  className='max-w-sm'
                                  placeholder="Enter here"
                              ...
                              <FormDescription>
                                The access point entered will be used for access point mode for controller
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                  ...
                  <div className="flex flex-col prose min-h-[80vh] justify-center items-center container">
                    <p className='text-muted-foreground'>The controller&apos;s code will be generated here</p>
